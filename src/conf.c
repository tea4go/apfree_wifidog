...
//>>> liudf added 20160114 内部可信域
const char    *g_inner_trusted_domains = "www.ztesoft.com,oa.ztesoft.com,fol.ztesoft.com,mail.ztesoft.com";
...
config_parse_token(const char *cp, const char *filename, int linenum)
{
    ...
    debug(LOG_ERR, "%s:%d：错误的参数(%s)", filename, linenum, cp);
    return oBadOption;
}
...
{
    debug(LOG_DEBUG, "parse_auth_server()");
    char *host = NULL,
         *path = NULL,
		 *text = NULL,
    ...
    while (memset(line, 0, MAX_BUF) && fgets(line, MAX_BUF - 1, file) && (strchr(line, '}') == NULL)) {
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
			...
			trim(text,' ');
			trim(text,'\t');
            debug(LOG_INFO, "= 解析子内容：  %-18s = [%s]", p1, text);
            ...
            case oAuthServSSLAvailable:
                ssl_available = parse_boolean_value(text);
                if (ssl_available < 0) {
                    debug(LOG_WARNING, "传入的参数值错误，应该是：yes 或 no。AuthServSSLAvailable在文件位置%s:%d的内容为[%s]" , filename,*linenum,text);
                    exit(-1);
                }
                break;
            case oBadOption:
            default:
                debug(LOG_ERR, "传入错误参数，进程将退出。位置：%s:%d", filename,*linenum );
                exit(-1);
                break;
            }
			if (text) {
    ...
    debug(LOG_DEBUG, "增加Auth服务器(%s://%s:%d%s)", (ssl_available?"https":"http"), host,(ssl_available?ssl_port:http_port), path);
    ...
    debug(LOG_DEBUG, "parse_auth_server() : end");
}
...
{
    debug(LOG_DEBUG, "parse_firewall_ruleset()");
    char line[MAX_BUF], *p1, *p2;
    int opcode;

    debug(LOG_DEBUG, "= 增加防火墙规则 [%s]", ruleset);
    ...
    while (memset(line, 0, MAX_BUF) && fgets(line, MAX_BUF - 1, file) && (strchr(line, '}') == NULL)) {
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            /* Get opcode */
            opcode = config_parse_token(p1, filename, *linenum);
            debug(LOG_DEBUG, "= 解析子内容：%-20s = [%s]", p1, p2);
            ...
            case oBadOption:
            default:
                debug(LOG_ERR, "错误的参数，在文件 %s:%d",filename,*linenum);
                debug(LOG_ERR, "进程将退出。");
                exit(-1);
                break;
            }
    ...
    debug(LOG_DEBUG, "= 增加防火墙规则 [%s] ... OK", ruleset);
    debug(LOG_DEBUG, "parse_firewall_ruleset() : end");
}
...
_parse_firewall_rule(const char *ruleset, char *leftover)
{
    ...
    //debug(LOG_ERR, "===> [%s] [%s] %d", token, leftover,finished);
    ...
    if (!strcasecmp(token, "block") || finished) {
        ...
        debug(LOG_ERR, "无效的规则类型[%s]，应该是block|drop|allow|log|ulog", token);
        return -1;
    }
    ...
    if (strncmp(leftover, "tcp", 3) == 0 || strncmp(leftover, "udp", 3) == 0 || strncmp(leftover, "icmp", 4) == 0) {
        protocol = leftover;
        TO_NEXT_WORD(leftover, finished);
        //debug(LOG_ERR, "===> [%s] [%s] %d", protocol, leftover,finished);
    }

    /* Get the optional port or port range */
    if (strncmp(leftover, "port", 4) == 0) {
        TO_NEXT_WORD(leftover, finished);
        //debug(LOG_ERR, "===> [%s] %d", leftover,finished);

        /* Get port now */
        port = leftover;
        TO_NEXT_WORD(leftover, finished);
        //debug(LOG_ERR, "===> [%s] [%s] %d", port, leftover,finished);

        for (i = 0; *(port + i) != '\0'; i++)
            if (!isdigit((unsigned char)*(port + i)) && ((unsigned char)*(port + i) != ':'))
                all_nums = 0;   /*< No longer only digits */
        if (!all_nums) {
            //debug(LOG_ERR, "无效的规则[%s]，端口错误[%s]", ruleset, port);
            return -3;          /*< Fail */
        }
    }
    ...
    if (!finished) {
        ...
        other_kw = leftover;
        TO_NEXT_WORD(leftover, finished);
        //debug(LOG_ERR, "===> [%s] [%s] %d", other_kw, leftover,finished);
        ...
        //debug(LOG_ERR, "===> [%s] %d", leftover,finished);
        if (!finished) {
            debug(LOG_WARNING, "成功解析规则后忽略后缀字符串[%s]", leftover);
        }
    }
    ...
    debug(LOG_DEBUG, "= 增加防火墙规则 %s %s协议 %s端口 给 %s 地址。", token, tmp->protocol?tmp->protocol:"所有", tmp->port?tmp->port:"所有", tmp->mask);
...
{
    debug(LOG_DEBUG, "config_read()");
    FILE *fd;
    char line[MAX_BUF] = {0}, *s, *p1, *p2, *text=NULL, *rawarg=NULL;
    int linenum = 0, opcode, value;
    size_t len;

    debug(LOG_NOTICE, "打开配置文件 '%s'", filename);

    if (!(fd = fopen(filename, "r"))) {
        debug(LOG_ERR, "不能打开配置文件 '%s'，进程将退出。", filename);
        exit(1);
    }
    ...
    while (!feof(fd) && fgets(line, MAX_BUF, fd)) {
        ...
        if (p1 && p1[0] != '\0') {
            ...
            if ((strncmp(s, "#", 1)) != 0) {
				...
				trim(text,' ');
				trim(text,'\t');
                debug(LOG_INFO, "= 解析关键字：%-20s = [%s]", s, text);

                switch (opcode) {
                case oDeltaTraffic:
                    config.deltatraffic = parse_boolean_value(text);
                    break;
                case oDaemon:
                    if (config.daemon == -1 && ((value = parse_boolean_value(text)) != -1)) {
                        config.daemon = value;
                        if (config.daemon > 0) {
                            debugconf.log_stderr = 0;
                        } else {
                ...
                case oExternalInterface:
                    config.external_interface = safe_strdup(text);
                    break;
                case oGatewayID:
                    config.gw_id = safe_strdup(text);
                    break;
                case oGatewayInterface:
                    config.gw_interface = safe_strdup(text);
                    break;
                case oGatewayAddress:
                    config.gw_address = safe_strdup(text);
                    break;
                case oGatewayPort:
                    sscanf(text, "%d", &config.gw_port);
                    break;
                case oAuthServer:
                    parse_auth_server(fd, filename, &linenum);
                    break;
                case oFirewallRuleSet:
                    parse_firewall_ruleset(text, fd, filename, &linenum);
                    break;
                case oTrustedMACList:
                    parse_trusted_mac_list(text);
                    break;
                case oPopularServers:
                    parse_popular_servers(rawarg);
                    break;
                case oHTTPDName:
                    config.httpdname = safe_strdup(text);
                    break;
                case oHTTPDMaxConn:
                    sscanf(text, "%d", &config.httpdmaxconn);
                    break;
                case oHTTPDRealm:
                    config.httpdrealm = safe_strdup(text);
                    break;
                case oHTTPDUsername:
                    config.httpdusername = safe_strdup(text);
                    break;
                case oHTTPDPassword:
                    config.httpdpassword = safe_strdup(text);
                    break;
                case oCheckInterval:
                    sscanf(text, "%d", &config.checkinterval);
                    break;
                case oWdctlSocket:
                    free(config.wdctl_sock);
                    config.wdctl_sock = safe_strdup(text);
                    break;
                case oClientTimeout:
                    sscanf(text, "%d", &config.clienttimeout);
                    break;
                case oSyslogFacility:
                    sscanf(text, "%d", &debugconf.syslog_facility);
                    break;
                case oHtmlMessageFile:
                    config.htmlmsgfile = safe_strdup(text);
                    break;
                case oProxyPort:
                    sscanf(text, "%d", &config.proxy_port);
                    break;
                case oSSLCertPath:
                    config.ssl_certs = safe_strdup(text);
#ifndef USE_CYASSL
                    debug(LOG_WARNING, "SSLCertPath已设置，但不编译SSL。忽略！");
#endif
                    break;
                case oSSLPeerVerification:
                    config.ssl_verify = parse_boolean_value(text);
                    if (config.ssl_verify < 0) {
                        debug(LOG_WARNING, "参数错误，值应该是 'yes' 或 'no'，SSLPeerVerification在文件%s:%d位置内容为[%s]，进程将退出。", linenum, filename,text);
                        exit(-1);
                    }
#ifndef USE_CYASSL
                    debug(LOG_WARNING, "SSLPeerVerification已设置，但不编译SSL。忽略！");
#endif
                    break;
                case oSSLAllowedCipherList:
                    config.ssl_cipher_list = safe_strdup(text);
#ifndef USE_CYASSL
                    debug(LOG_WARNING, "SSLAllowedCipherList已设置，但不编译SSL。忽略！");
#endif
                    break;
                case oSSLUseSNI:
                    config.ssl_use_sni = parse_boolean_value(text);
                    if (config.ssl_use_sni < 0) {
                        debug(LOG_WARNING, "参数错误，值应该是 'yes' 或 'no'，SSLUseSNI在文件%s:%d位置内容为[%s]，进程将退出。", linenum, filename,text);
                        exit(-1);
                    }
#ifndef USE_CYASSL
                    debug(LOG_WARNING, "SSLUseSNI已设置，但不编译SSL。忽略！");
#else
#ifndef HAVE_SNI
                    debug(LOG_WARNING, "SSLUseSNI已设置，但未启用CyaSSL SNI。忽略！");
#endif
#endif
                    break;
                // >>> liudf added 20151224
                ...
                case oUntrustedMACList:
                    parse_untrusted_mac_list(text);
                    break;
                case oJsFilter:
                    config.js_filter = parse_boolean_value(text);
                    break;
                case oPoolMode:
                    config.pool_mode = parse_boolean_value(text);
                    break;
                case oThreadNumber:
                    sscanf(text, "%hd", &config.thread_number);
                    break;
                case oQueueSize:
                    sscanf(text, "%hd", &config.queue_size);
                    break;
                case oWiredPassed:
                    config.wired_passed = parse_boolean_value(text);
                    break;
                case oParseChecked:
                    config.parse_checked = parse_boolean_value(text);
                    break;
                case oTrustedIpList:
                    add_trusted_ip_list(rawarg);
                    break;
                case oNoAuth:
                    config.no_auth = parse_boolean_value(text);    
                    break;
                case oGatewayHttpsPort:
                    sscanf(text, "%hu", &config.https_server->gw_https_port);
                    break;
                case oWorkMode:
                    sscanf(text, "%hu", &config.work_mode);
                    break;
                case oUpdateDomainInterval:
                    sscanf(text, "%d", &config.update_domain_interval);
                    break;
                // <<< liudf added end
                case oBadOption:
                    /* FALL THROUGH */
    ...
    debug(LOG_NOTICE, "= 线程池模式       ... %s ", config.pool_mode?"是":"否");
    if(config.pool_mode){
        debug(LOG_NOTICE, "=     线程数       ... %d ", config.thread_number);
        debug(LOG_NOTICE, "=     队列数       ... %d ", config.queue_size);	
	}
    debug(LOG_NOTICE, "= Ping间隔(秒)     ... %d ", config.checkinterval);
    debug(LOG_NOTICE, "= Counters间隔(秒) ... %d ", config.checkinterval+3);
    debug(LOG_NOTICE, "= 心跳间隔(秒)     ... %d ", config.checkinterval+7);
    debug(LOG_NOTICE, "= 解析域名(秒)     ... %d ", config.update_domain_interval * config.checkinterval);
    debug(LOG_NOTICE, "= 终端超时(分)     ... %d ", (config.clienttimeout*config.checkinterval)/60);
    debug(LOG_NOTICE, "= 有线直接通过     ... %s ", config.wired_passed?"是":"否");
    debug(LOG_NOTICE, "= 工作模式         ... %d ", config.work_mode);
    debug(LOG_NOTICE, "= 是否自动重定向   ... %s ", config.js_filter?"是":"否");

    // liudf added 20160125
    // parse inner trusted domain string
    debug(LOG_INFO, "# 解析内部可信域名");
    parse_inner_trusted_domain_string(g_inner_trusted_domains);

    if (config.httpdusername && !config.httpdpassword) {
        debug(LOG_ERR, "Web服务器需要设置密码，进程将退出。");
        exit(-1);
    }

    fclose(fd);
    debug(LOG_DEBUG, "配置文件解析完成 '%s'", filename);
...
{
    debug(LOG_DEBUG, "从终端列表里删除Mac地址 %s 的节点", mac);
...
{
    debug(LOG_DEBUG, "增加Mac地址 %s 到漫游名单域名列表。", mac);
    if (config.roam_maclist == NULL) {
        ...
        if (!skipmac) {
            ...
            debug(LOG_ERR,"Mac地址 %s 已经在漫游名单域名列表里了。",mac);
        }
    }
}
...
remove_trusted_mac(const char *mac)
{
    ...
    debug(LOG_DEBUG, "从白名单域名列表里删除Mac地址 %s 。", mac);
...
add_trusted_mac(const char *mac)
{
    ...
    debug(LOG_DEBUG, "增加Mac地址 %s 到白名单域名列表。", mac);
    ...
    if (config.trustedmaclist == NULL) {
        ...
        if (!skipmac) {
            ...
            debug(LOG_ERR,"Mac地址 %s 已经在白名单域名列表里了。",mac);
        }
    }
...
add_untrusted_mac(const char *mac)
{
    ...
    debug(LOG_DEBUG, "增加Mac地址 %s 到黑名单域名列表。", mac);
    ...
    if (config.mac_blacklist == NULL) {
        ...
        if (!skipmac) {
            ...
            debug(LOG_ERR,"Mac地址 %s 已经在黑名单域名列表里了。",mac);
        }
    }
}
...
remove_untrusted_mac(const char *mac)
{
    ...
    debug(LOG_DEBUG, "从黑名单域名列表里删除Mac地址 %s 。", mac);
    if(config.mac_blacklist == NULL)
        return;
...
parse_mac_list_action(const char *ptr, mac_choice_t which, int action)
{
    ...
    if(is_valid_mac(ptr)) {
        ...
        debug(LOG_DEBUG, "%sMac地址 %s 到列表。", action?"增加":"删除", ptr);
        if(action)
            add_mac(ptr, which);
        else
            remove_mac(ptr, which);
        debug(LOG_DEBUG, "解析Mac地址 [%s] ... OK", ptr);
        return;
    }
    ...
    while ((possiblemac = strsep(&ptrcopy, ","))) {
        ...
        if (is_valid_mac(possiblemac)) {
            debug(LOG_DEBUG, "%sMac地址 %s 到列表。", action?"增加":"删除", possiblemac);
            if(action)
                add_mac(possiblemac, which);
            else
                remove_mac(possiblemac, which);
        } else
            debug(LOG_ERR, "[%s]是一个无效的Mac地址。", possiblemac);
    }
...
parse_popular_servers(const char *ptr)
{
    ...
    debug(LOG_DEBUG, "解析流行的服务器域名 [%s] ，用来判断互联网状态。", ptr);
    ...
    while ((hostname = strsep(&ptrcopy, ","))) {  /* hostname does *not* need allocation. strsep
        ...
        debug(LOG_DEBUG, "# 增加流行的域名 %s 到列表。", hostname);
        add_popular_server(hostname);
    }

    free(pt);

    debug(LOG_DEBUG, "解析流行的服务器域名 [%s] ... OK", ptr);
}
...
__add_domain_common(const char *domain, trusted_domain_t which)
{
    ...
    if(which == USER_TRUSTED_DOMAIN) {    
        debug(LOG_DEBUG, "增加用户域名(%s)",domain);
        if (config.domains_trusted == NULL) {
        ...
        debug(LOG_DEBUG, "增加内置域名(%s)",domain);
        if (config.inner_domains_trusted == NULL) {
            p = (t_domain_trusted *)safe_malloc(sizeof(t_domain_trusted));
            p->domain = safe_strdup(domain);
            p->next = NULL;
        ...
        debug(LOG_DEBUG, "增加泛域名(%s)",domain);
        if (config.pan_domains_trusted == NULL) {
            p = (t_domain_trusted *)safe_malloc(sizeof(t_domain_trusted));
            p->domain = safe_strdup(domain);
            p->next = NULL;
...
{
    debug(LOG_DEBUG, "parse_domain_string_common_action()");
    char *ptrcopy = NULL, *pt = NULL;
    char *hostname = NULL;
    char *tmp = NULL;

    debug(LOG_DEBUG, "=Parsing string [%s] for trust domains(信任的域名)", ptr);
    ...
    while ((hostname = strsep(&ptrcopy, ","))) {  /* hostname does *not* need allocation. strsep
        ...
        if(action) { // 1: add
            ...
            debug(LOG_DEBUG, "=  Delete trust domain [%s] from list", hostname);
    ...
    UNLOCK_DOMAIN();
    free(pt);
    debug(LOG_DEBUG, "parse_domain_string_common_action() : end");
}
...
void add_trusted_ip_list(const char *ptr)
{
    ...
    debug(LOG_DEBUG, "解析IP地址 [%s]", ptr);
    
    p = add_domain_common("iplist", USER_TRUSTED_DOMAIN);
    if(p == NULL) {
        debug(LOG_ERR, "不可能：添加iplist域名失败)");
        debug(LOG_DEBUG, "解析IP地址 [%s] ... OK", ptr);
        return;
    }
    ...
    while ((ip = strsep(&ptrcopy, ","))) {  /* ip does *not* need allocation. strsep
        ...
        debug(LOG_DEBUG, "增加IP地址 %s 到列表。", ip);        
...
validate_popular_servers(void)
{
    if (config.popular_servers == NULL) {  
        ...
        add_popular_server("www.yjsoft.com");
    }
}
...
{
    debug(LOG_DEBUG, "mark_auth_server_bad() : 标识Auth服务器(%s)为离线，并移入尾部。",bad_server->authserv_hostname);
    t_auth_serv *tmp;

	if (bad_server==NULL) {
		bad_server = config.auth_servers;
    }
