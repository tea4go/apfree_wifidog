...
ipset_do_command(const char *format, ...)
{
    ...
    if (rc != 0) {
        ...
        if (fw_quiet == 0)
            debug(LOG_ERR, "= 命令执行失败(%d)：%s", rc, cmd);
        else if (fw_quiet == 1)
            debug(LOG_DEBUG, "= 命令执行失败(%d)：%s", rc, cmd);
    }
...
iptables_do_command(const char *format, ...)
{
    ...
    f_fw_script_write(cmd);
    if(fw_quiet == 2) {
        debug(LOG_DEBUG, "写入命令文件：%s", cmd);
        free(cmd);
        return 0;
    }

    debug(LOG_INFO, "执行命令：%s", cmd);
...
iptables_load_ruleset(const char *table, const char *ruleset, const char *chain)
{
    ...
    debug(LOG_DEBUG, "将规则集 %s 载入表 %s，链%s", ruleset, table, chain);

    for (rule = get_ruleset(ruleset); rule != NULL; rule = rule->next) {
        cmd = iptables_compile(table, chain, rule);
        if (cmd != NULL) {
            debug(LOG_DEBUG, "将规则 %s 载入表 %s，链 %s", cmd, table, chain);
            iptables_do_command(cmd);
        }
        free(cmd);
    }

    debug(LOG_DEBUG, "规则集 %s 载入表 %s ，链 %s", ruleset, table, chain);
}
...
{
    debug(LOG_DEBUG, "fw_set_pan_domains_trusted()");
    const s_config *config;
    t_domain_trusted *domain_trusted = NULL;
    FILE *fd_ipset = NULL;
    char f_ipset_name[128] = {0};
    ...
    snprintf(f_ipset_name, 128, "%s/%s", DNSMASQ_CONF_D, CHAIN_IPSET_TDOMAIN);
    debug(LOG_DEBUG, "fw_set_pan_domains_trusted() : 打开DNS文件(%s)",f_ipset_name);
    fd_ipset = fopen(f_ipset_name, "w");
    if(fd_ipset == NULL) {
        debug(LOG_ERR, "fw_set_pan_domains_trusted() : 打开DNS文件失败(%s)",f_ipset_name);
        return;
    }
    ...
    for (domain_trusted = config->pan_domains_trusted; domain_trusted != NULL; domain_trusted = domain_trusted->next) {
        ...
        debug(LOG_DEBUG, "fw_set_pan_domains_trusted() : 增加泛域名(ipset=/.%s/%s)",domain_trusted->domain, CHAIN_IPSET_TDOMAIN);
    }
    ...
    debug(LOG_DEBUG, "fw_set_pan_domains_trusted() : iptables_flush_ipset"); 
    iptables_flush_ipset(CHAIN_IPSET_TDOMAIN);

    debug(LOG_DEBUG, "fw_set_pan_domains_trusted() : 重启DNS服务器(/etc/init.d/dnsmasq restart)");
    execute("/etc/init.d/dnsmasq restart", 1);        

    debug(LOG_DEBUG, "fw_set_pan_domains_trusted() : end");
}
...
iptables_fw_destroy(void)
{
    ...
    debug(LOG_DEBUG, "Destroying our iptables entries(删除防火墙规则)");
     ...
     *
     */
    debug(LOG_DEBUG, "A - Destroying chains in the MANGLE table");
    // liudf added 20160106
     ...
     *
     */
    debug(LOG_DEBUG, "B - Destroying chains in the NAT table");
     ...
     *
     */
    debug(LOG_DEBUG, "C - Destroying chains in the FILTER table");
...
iptables_fw_destroy_mention(const char *table, const char *chain, const char *mention)
{
    ...
    debug(LOG_DEBUG, "=  准备删除所有从 %s.%s 提到的 %s 链路。", table, chain, victim);
        ...
        while (fgets(line, sizeof(line), p)) {
            ...
            if (strstr(line, victim)) {
                ...
                if (sscanf(line, "%9[0-9]", rulenum) == 1) {
                    /* Delete the rule: */
                    debug(LOG_DEBUG, "从 %s.%s 删除规则%s，因为它用到 %s", table, chain,rulenum, victim);
...
__get_client_name(t_client *client)
{
    ...
    debug(LOG_DEBUG, "分析DHCP动态分配IP的文件，执行命令：%s", cmd);
    if((f_dhcp = popen(cmd, "r")) != NULL) {
        ...
        debug(LOG_INFO, "通过IP地址(%s)获得终端名称[%s]", client->ip,name);
    }
}
...
iptables_fw_counters_update(void)
{
    ...
    debug(LOG_DEBUG, "get_counters() : 读取上行(outgoing)的流量。命令行：%s",script);
	output = popen(script, "r");
    if (!output) {
        debug(LOG_ERR, "执行popen命令失败。命令行：%s，错误码：%d，原因：%s", script,errno,strerror(errno));
        free(script);
		return -1;
    }
    free(script);
    ...
    while (output && !(feof(output))) {
        ...
        if (2 == rc && EOF != rc) {
            ...
            if (!inet_aton(ip, &tempaddr)) {
                debug(LOG_WARNING, "忽略：应该读一个IP地址，而是得到[%s]", ip);
                continue;
            }
            debug(LOG_DEBUG, "get_counters() : 从 %s 读取上行(outgoing)的流量：%llu字节", ip, counter);
            LOCK_CLIENT_LIST();
            if ((p1 = client_list_find_by_ip(ip))) {
                if ((p1->counters.outgoing - p1->counters.outgoing_history) < counter) 
				{
                    ...
                    //p1->counters.last_updated = time(NULL);  //未验证
					gettimestr(p1->counters.last_updated,date_str,sizeof(date_str)-1);
                    debug(LOG_DEBUG, "get_counters() : 从 %s 读取上行(outgoing)的流量：%llu字节, 新增流量：%llu字节，最后更新日期：%s", ip,
                          p1->counters.outgoing,p1->counters.outgoing_delta, date_str);
                    p1->is_online = 1;
                } 
                ...
                if(p1->wired == -1) {
                    p1->wired = 0;//is_device_wired(p1->mac); 这个功能没实现，直接去掉。
                }
                UNLOCK_CLIENT_LIST();
            } else {
                UNLOCK_CLIENT_LIST();
                debug(LOG_ERR,"在客户端列表中找不到 %s，除非网关崩溃，否则不应该发生！",ip);

                debug(LOG_ERR, "预防性地删除表 %s 中 %s 的防火墙规则", CHAIN_OUTGOING,ip);
                iptables_fw_destroy_mention("mangle", CHAIN_OUTGOING, ip);
                
				debug(LOG_ERR, "预防性地删除表 %s 中 %s 的防火墙规则", CHAIN_INCOMING,ip);
    ...
    iptables_insert_gateway_id(&script);
    debug(LOG_DEBUG, "get_counters() : 读取下行(incoming)的流量。命令行：%s",script);
	output = popen(script, "r");
    if (!output) {
        debug(LOG_ERR, "执行popen命令失败。命令行：%s，错误码：%d，原因：%s", script,errno,strerror(errno));
        free(script);
    ...
    while (output && !(feof(output))) {
        ...
        if (2 == rc && EOF != rc) {
            ...
            debug(LOG_DEBUG, "get_counters() : 从 %s 读取下行(incoming)的流量：%llu字节", ip, counter);
            LOCK_CLIENT_LIST();
            if ((p1 = client_list_find_by_ip(ip))) {
                if ((p1->counters.incoming - p1->counters.incoming_history) < counter) 
				{
                    ...
                    //p1->counters.last_updated = time(NULL); //未验证
					gettimestr(p1->counters.last_updated,date_str,sizeof(date_str)-1);
                    debug(LOG_DEBUG, "get_counters() : 从 %s 读取下行(incoming)的流量：%llu字节, 新增流量：%llu字节，最后更新日期：%s", ip, 
						p1->counters.incoming, p1->counters.incoming_delta, date_str);
                    p1->is_online = 1;
                }
                ...
                debug(LOG_ERR, "预防性地删除表%s中%s的防火墙规则", CHAIN_OUTGOING,ip);
                iptables_fw_destroy_mention("mangle", CHAIN_OUTGOING, ip);
                
				debug(LOG_ERR, "预防性地删除表%s中%s的防火墙规则", CHAIN_INCOMING,ip);
                iptables_fw_destroy_mention("mangle", CHAIN_INCOMING, ip);
            }
        }
    }
